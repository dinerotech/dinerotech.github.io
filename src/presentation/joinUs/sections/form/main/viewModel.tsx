...
import { JOIN_US_ENDPOINT } from "@/domain/forms/constants";
  ...
  const handleSubmit = React.useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const token = await executeRecaptcha("submit_join_us_form");
        formData.append("captchaToken", token);
        formData.append("submittedAt", new Date().toISOString());
        const result = await SubmitFormUseCase(JOIN_US_ENDPOINT, formData);
        if (result) {
